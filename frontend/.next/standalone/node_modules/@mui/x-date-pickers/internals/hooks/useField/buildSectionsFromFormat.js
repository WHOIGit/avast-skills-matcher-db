...
const expandFormat = ({
  ...
  while (nextFormat !== prevFormat) {
    ...
    if (formatExpansionOverflow < 0) {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X: The format expansion seems to be in an infinite loop. Please open an issue with the format passed to the component.' : (0, _formatErrorMessage2.default)(154));
    }
  }
  return nextFormat;
};
...
const createSection = ({
  ...
  if (token === '') {
    throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X: Should not call `commitToken` with an empty token' : (0, _formatErrorMessage2.default)(155));
  }
  ...
  if (hasLeadingZerosInInput) {
    if (hasLeadingZerosInFormat) {
      ...
      if (sectionConfig.maxLength == null) {
        throw new Error(process.env.NODE_ENV !== "production" ? `MUI X: The token ${token} should have a 'maxLength' property on it's adapter` : (0, _formatErrorMessage2.default)(156, token));
      }
